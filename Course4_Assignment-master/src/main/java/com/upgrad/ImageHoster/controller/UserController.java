...
public class UserController {
	...
	private static final String USERNAME_VALIDATION_ERROR = "needs to be 6 characters or longer ";
	private static final String PASSWORD_VALIDATION_ERROR = "needs to be 6 characters or longer";
	private static final String USER_EXIST_VALIDATION_ERROR = "username has been registered";

	private static Predicate<String> FIELD_LENGTH_VALIDATOR = field -> isEmpty(field)
			|| field.length() < MIN_FIELD_LENGTH;
	...
	public String signUpUser(@RequestParam("username") String username, @RequestParam("password") String password,
		...
		Errors errors = validateUserData(username, password);

		if (errors.hasErrors()) {
			Map<String, String> errorMap = errors.getAllErrors()
					.stream()
	...
	private Errors validateUserData(String username, String password) {
		BindingResult errors = new BeanPropertyBindingResult(User.class, "user");

		if (userService.getByName(username) != null) {      // User Already Exist
			errors.reject("username", String.format(USER_EXIST_VALIDATION_ERROR, username));
		} else {
			if (FIELD_LENGTH_VALIDATOR.test(username)) {         // Username must be 6 characters Long
				errors.reject("username", USERNAME_VALIDATION_ERROR);
			}

			if (FIELD_LENGTH_VALIDATOR.test(password)) {          // Password must be 6 characters long
				errors.reject("password", PASSWORD_VALIDATION_ERROR);
			}
		}
	...
	public String editUserProfile(@RequestParam("description") String description, @RequestParam("file")
