...
public class ImageManager extends SessionManager {
    ...
    public void deleteImage(final int id) {
        Session session = openSession();
        Query query = session.createQuery("Delete from " + Comment.class.getName() + " where image_id=:id");
        query.setParameter("id", id);
        query.executeUpdate();

        query = session.createQuery("Delete from " + Image.class.getName() + " where id=:id");
        query.setParameter("id", id);
        query.executeUpdate();
        commitSession(session);
    }
